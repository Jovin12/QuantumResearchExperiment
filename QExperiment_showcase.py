...
def page():
    ...
    if "fidelity_number" not in st.session_state: 
        st.session_state.fidelity_number = 0.0
    if "model" not in st.session_state: 
        st.session_state.model = None

    with st.container(border=True, key='green_container'):
        st.html(f"<style>{home.css}</style>")
        # Note: st.set_page_config must be the very first st command in the entry point. 
        # If this is a multipage app, it should be in the main file, not the page function.
        ...
        if fidelity_selection == "Use_PRETrained Model":
            ...
            res = requests.get(FASTAPI_URL)
            response = requests.get(FASTAPI_URL)

            if response.status_code == 200:
                # Convert the raw bytes from the response into a JIT model
            ...
            if not st.session_state.experiment_finished:
                with st.container(border=True):
                    ...
                    if fidelity_selection == "Simple Fidelity":
                        st.session_state.fidelity_number = qiskit_circuit_general.simpleFidelityEstimator(st.session_state.main_qc)
                        fidelity_bar.progress(100, text=f"Fidelity: {st.session_state.fidelity_number}")
                        ...
                        fidelity_bar.progress(100, text=f"QBound finished: ±{st.session_state.fidelity_error_bound}")
                    ...
                    st.rerun() # Refresh to show results and the upload form

            # --- 7. DISPLAY RESULTS & UPLOAD FORM ---
            if st.session_state.experiment_finished:
                st.success(f"Analysis Complete! \n Your Fidelity score is: {st.session_state.fidelity_error_bound}")
                st.pyplot(qiskit_circuit_general.display_circuit(st.session_state.main_qc))
                ...
                # Wrap in a Form to prevent reruns while typing name
                with st.form("db_upload_form"):
                    ...
                    if upload_btn:
                        if not model_name_input:
                            ...
                            st.error("No JIT model found from QuBound.")
                        else:
                            try:
                                # Convert model to JIT
                                model_jit = torch.jit.script(st.session_state.model)
                                buffer = io.BytesIO()
                                ...
                                files = {
                                    "file": (f"{model_name_input}.pt", buffer, "application/octet-stream")
                                }
        ...
        # Reset Button to clear the session and run a new circuit
        if st.session_state.experiment_finished:
            if st.button("Start New Experiment"):
                for key in ["submitted", "verified", "experiment_finished", "model"]:
